...
var _messenger_profile = require('./messenger_profile');
...
class Messenger extends _events2.default {
  ...
  buildURL(pathname, queryParams) {
    ...
    const obj = {
      ...
      pathname: `/${_constants.FB_API_VERSION}/${pathname}`,
      query
    };
  ...
  get(url) {
    ...
    }).then(response => response.json()).then(res => this.constructor.handleError(res));
  }
  ...
  getUser(id, fields) {
    ...
    const fieldsArray = fields || ['first_name', 'last_name', 'profile_pic', 'locale', 'timezone', 'gender', 'is_payment_enabled', 'last_ad_referral'];
  ...
  send(payload, id, tag = '') {
    ...
    if (tag && _constants.TAGS.indexOf(tag) === -1) {
  ...
  */
  ...
  setGetStarted(payload) {
    if (payload.length > _constants.GET_STARTED_LIMIT) {
      throw new Error(`Get Started payload limit is ${_constants.GET_STARTED_LIMIT}.`);
    }
    const params = {
      get_started: { payload }
    };
  ...
  setDomainWhitelist(payload) {
    ...
    if (payload.length > _constants.WHITELISTED_DOMAIN_MAX) {
      throw new Error(`You may only whitelist ${_constants.WHITELISTED_DOMAIN_MAX} domains.`);
    }
    const params = {
...
exports.Button = _elements.Button;
exports.Address = _elements.Address;
...
exports.Video = _attachments.Video;
exports.File = _attachments.File;
exports.ButtonTemplate = _templates.ButtonTemplate;
